<h1 class="navbar-brand">
    <a href="<?php echo esc_url(site_url('/')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>">
        ...
        <?php 
            ...
            $text_logo_typography = maacuni_option('text-logo-typography', false, false);
            ...
            if (maacuni_option('logo-type', false, 'logo')) : 

                // site logo
                $fallback_logo = maacuni_option('logo', 'url', get_template_directory_uri() . '/images/logo.jpg');
                $fallback_retina_logo = get_template_directory_uri() . '/images/logo2x.png';

                $default_logo = maacuni_option('logo', 'url', $fallback_logo);
                $default_ratina_logo = maacuni_option('retina-logo', 'url', $fallback_retina_logo);
                ...
                // mobile logo option
                if (maacuni_option('mobile-logo')) :
                    $site_mobile_logo = maacuni_option('mobile-logo', 'url', $fallback_logo);
                endif;
                ...
                if (maacuni_option('retina-mobile-logo', 'url')) :
                    $site_retina_mobile_logo = maacuni_option('retina-mobile-logo', 'url', $fallback_retina_logo);
                endif;


                // site logo display
                if (function_exists('rwmb_meta') && rwmb_meta('maacuni_page_logo', 'type=image_advanced')) :
                    $page_logo = rwmb_meta('maacuni_page_logo', 'type=image_advanced');
                ...
                // mobile logo display
                if (function_exists('rwmb_meta') && rwmb_meta('maacuni_page_mobile_logo', 'type=image_advanced')) :
                    $page_mobile_logo = rwmb_meta('maacuni_page_mobile_logo', 'type=image_advanced');
                ...
                <span class="text-logo" style="<?php echo esc_attr($text_logo_color .' '. $text_logo_fontsize .' '. $text_logo_font_family .' '. $text_logo_font_weight .' '. $text_logo_line_height) ?>">
                    <?php if (maacuni_option('text-logo')) : ?>
                        <?php echo esc_html(maacuni_option('text-logo')); ?>
                    <?php else : ?>
